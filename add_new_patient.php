...
$query_verify_email = "SELECT * FROM patients  WHERE email ='$email'";
        ...
        if (mysql_num_rows($result_verify_email) == 0) {
		 // Create a unique  activation code:
            ...
            $query_insert_user = "INSERT INTO patients (fname, lname, gender, type_of_blood, email, password, status, role, token, gp) 
								  VALUES ('$fname', '$lname', '$gender', '$blood', '$email', '$password', 1, 3, '$token', '$userid')";
            ...
            if (mysql_affected_rows() == 1) { //If the Insert Query was successfull.
				...
				if(mysql_num_rows($result) == 1) {
					...
					// ADD TO PATIENT INFO AS WELL 
					$query_insert_users_info = "INSERT INTO patient_info (patients) 
											  VALUES ('$id')";
					...
					*/
				echo(msg(1,"New patient has been successfully added. Add patient DETAILS? (Eg: Blood pressure etc) <a style='color:#fff' class='btn btn-info' href='update_patient.php?userid=$id'>YES!</a>&nbsp;&nbsp;<a style='color:#fff' class='btn btn-warning' href='doctor.php'>NO!</a>"));
			}
